...
def modelo_elasticidad(df):
    ...
    if len(df_agg) < 3 or df_agg["precio_unitario"].nunique() < 2:
        return -1.0, np.nan, np.nan, np.nan, len(df_agg), "Insuficiente variabilidad (Usando Base -1.0)"
...
# ==========================================
# COMPARTIR FILTROS GLOBALES EN CASCADA
# ==========================================
if df is not None:
    # Contenedor global de filtros ocultos o sutiles si se necesitan sincronizar
...
# ==========================================
# VISTA 2: ELASTICIDAD (RECONECTADA Y FUNCIONAL)
# ==========================================
    ...
    if len(skus_disponibles2) == 0:
        ...
        beta, alfa, r2, pval, obs, diag = modelo_elasticidad(df_sku2)
        
        st.markdown("### Resumen Estadístico de la Regresión (Log-Log)")
        k1, k2, k3, k4 = st.columns(4)
        k1.metric("Elasticidad (Coeficiente Beta)", f"{beta:.4f}" if pd.notna(beta) else "N/A")
        k2.metric("Coeficiente de Determinación (R²)", f"{r2:.4f}" if pd.notna(r2) else "N/A")
        k3.metric("P-Valor (Significancia)", f"{pval:.4f}" if pd.notna(pval) else "N/A")
        k4.metric("Puntos de Precio Analizados", f"{obs}")
        
        st.markdown("#### Historial de Registros Filtrados para este SKU")
        st.dataframe(df_sku2[["tran_date", "prod_nbr", "qty", "precio_unitario", "net_sale", "categoria_est_socio"]].head(30), use_container_width=True)

# ==========================================
# VISTA 3: PRICING DINÁMICO (GRÁFICAS EN UN SOLO SNAPSHOT)
# ==========================================
    ...
    if len(skus_disponibles) == 0:
        ...
        # Coeficiente de elasticidad aplicado
        ...
        st.markdown("### Análisis Gráfico de Impacto (Snapshot Sincronizado)")
        g1, g2 = st.columns(2)
        
        # ---- GRÁFICA 1: LÍNEAS DE INGRESO (EJE Y COMPARTIDO) ----
        with g1:
            fig_l1 = go.Figure()
            ...
            # Forzar eje Y unificado para ver ambas líneas en un solo snapshot visual
            min_y_i = min(df_temporal["i_base"].min(), df_temporal["i_sim"].min()) * 0.9
            max_y_i = max(df_temporal["i_base"].max(), df_temporal["i_sim"].max()) * 1.1
            
            fig_l1.update_layout(
                title="Evolución de Ingresos Semanales ($)",
                xaxis_title="Semana",
                yaxis_title="Monto ($)",
                yaxis=dict(range=[min_y_i, max_y_i]),
                legend=dict(orientation="h", y=1.1)
            )
            st.plotly_chart(fig_l1, use_container_width=True)
            st.write(f"📝 El ingreso real acumulado fue de **${tot_i_real:,.2f}** vs **${tot_i_sim:,.2f}** proyectados.")

        # ---- GRÁFICA 2: LÍNEAS DE UNIDADES (EJE Y COMPARTIDO) ----
        with g2:
            fig_l2 = go.Figure()
            ...
            # Forzar eje Y unificado para unidades
            min_y_u = min(df_temporal["u_base"].min(), df_temporal["u_sim"].min()) * 0.9
            max_y_u = max(df_temporal["u_base"].max(), df_temporal["u_sim"].max()) * 1.1
            
            fig_l2.update_layout(
                title="Volumen de Unidades Semanales (Qty)",
                xaxis_title="Semana",
                yaxis_title="Unidades",
                yaxis=dict(range=[min_y_u, max_y_u]),
                legend=dict(orientation="h", y=1.1)
            )
            st.plotly_chart(fig_l2, use_container_width=True)
            st.write(f"📝 Volumen físico real: **{tot_u_real:,.0f}** piezas vs **{tot_u_sim:,.0f}** piezas proyectadas.")
